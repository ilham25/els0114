...
//{{ 2009. 3. 13  최육사	performer에 있는 이벤트 모두 빼오기 [임시]
bool KThreadManager::GetKEventAll( std::queue< KEventPtr >& queueEvent )
{
	{ // locking scope
		KLocker lock( m_csEventQueue );
		...
		{
			queueEvent.push( m_queEvent.front() );
			m_queEvent.pop();
		}

		if( queueEvent.empty()  &&  m_bTerminateReserved ) // 종료가 예약되었고, 처리할 데이터가 더이상 남지 않았다.
		{
			++m_nTerminateCount;
			// false를 리턴하면 스레드를 종료한다.
			return false;
		}
	} // locking scope
...
void KThreadManager::EndThread( DWORD dwTimeOut_/* = 10000 */ )
{
    KLocker lock( m_csVecThread );
    ...
    while( m_nTerminateCount < m_vecThread.size() && ::GetTickCount() - dwWaitTick < dwTimeOut_ )
    {
        ::Sleep( 20 );
    }
