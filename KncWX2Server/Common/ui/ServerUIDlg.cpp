...
void CServerUIDlg::GetCommandLineString()
{
	...
	{
		//{{ 2009. 7. 13  최육사	-s 옵션이 전달인자로 들어오면 db connection popup을 띄우자!
		if( std::wstring( pstrArgList[i] ) == L"-popup" )
		{
			bDBConnStrPopUp = true;
			continue; // 전달인자 리스트에서는 제외!
		}
		//}}

		m_vecCmdArgs.push_back( pstrArgList[i] );
		//ZeroMemory( pstrArgList[i], wcslen( pstrArgList[i] ) ); - 초기화의 의미가 없다 다른방법을 찾아보자
	}
