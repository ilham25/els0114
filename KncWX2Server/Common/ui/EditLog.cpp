		...
		)
{
	...
	//------------------------------------------------------------------- Max Line	
	int iLine = ::SendMessage( hEdit, EM_GETLINECOUNT , 0, 0 );
	if( iLine > MAX_EDIT_LOG_LINE_NUM )
	{
		int iLineIndex = ::SendMessage( hEdit, EM_LINEINDEX, iLine - MAX_EDIT_LOG_LINE_NUM - 1, 0 );
		::SendMessage( hEdit, EM_SETSEL, (WPARAM) 0, (LPARAM) iLineIndex );
		::SendMessage( hEdit, EM_REPLACESEL, (WPARAM) FALSE, (LPARAM) L"" );			
		::SendMessage( hEdit, EM_LINESCROLL, 0, (LPARAM) MAX_EDIT_LOG_LINE_NUM );
	}
	//------------------------------------------------------------------- Max Line
	return bAdded;
}
