#include "winsock2.h"
//#include "GarenaBillingTCPSession.h"
...
ImplementSingleton( KGarenaBillingIocp );
NiImplementRTTI( KGarenaBillingIocp, KThreadManager );
ImplPfID( KGarenaBillingIocp, PI_NULL );   // IOCP는 PfID 사용 안함.

KGarenaBillingIocp::KGarenaBillingIocp() 
: 
m_hIOCP( NULL ),
m_dwTerm(0L),
m_dwSockObjNum(0L)
...
void KGarenaBillingIocp::EndThread()
{
...
bool KGarenaBillingIocp::AssociateSocket( KSocketObjectPtr spSockObj )
{
...
bool KGarenaBillingIocp::AssociateSocket( SOCKET hSocket, ULONG_PTR CompKey )
{
    return ( CreateIoCompletionPort( (HANDLE) hSocket, m_hIOCP, CompKey, 0 ) != NULL );
}

bool KGarenaBillingIocp::PostStatus( ULONG_PTR CompKey, 
                         DWORD dwNumBytes /* = 0 */, 
                         OVERLAPPED* po /* = NULL */ )
{
...
bool KGarenaBillingIocp::GetStatus( ULONG_PTR* pCompKey,
                        PDWORD pdwNumBytes,
                        OVERLAPPED** ppo,
...
bool KGarenaBillingIocp::DeleteCompKey( IN DWORD dwKey_ )
{
    KLocker lock( m_csSockObj );
...
void KGarenaBillingIocp::OnIOCompleted( IN DWORD dwKey_, IN DWORD dwBytesTransfer_, IN OVERLAPPED* povl_ )
{
    if( povl_ == NULL )
    {
...
void KGarenaBillingIocp::OnClosedByLocal( IN DWORD dwKey_ )
{
    KSocketObjectPtr spSockObj = GetSockObj( dwKey_ );
...
size_t KGarenaBillingIocp::GetSockObjNum() const
{ 
    KLocker lock( m_csSockObj ); 
    return m_mapSockObj.size(); 
}

KSocketObjectPtr KGarenaBillingIocp::GetSockObj( DWORD dwKey_ )
{
    KLocker lock( m_csSockObj );
...
KThread* KGarenaBillingIocp::CreateThread()
{
    return new KGarenaBillingIOThread;
}

#undef max
#include <boost/random.hpp>

DWORD KGarenaBillingIocp::GenNewCompKey() const
{
    ...
    if( dwTerm > m_dwTerm )
    {
        KLocker lock( KGarenaBillingIocp::m_cs );
        m_dwTerm         = dwTerm;
        m_cTime          = CTime::GetCurrentTime();
...
ImplToStringW( KGarenaBillingIocp )
{
	...
	return stm_ << L" -- Max TRIOCP INFO --" << std::endl
		...
		<< L"  Key Gen Term        : " << KGarenaBillingIocp::m_dwTerm << std::endl
		<< L"  Occured Time        : " << (const wchar_t*)KGarenaBillingIocp::m_cTime.Format( _T("%Y-%m-%d %H:%M:%S") ) << std::endl
        << L"  socket object NUM   : " << KGarenaBillingIocp::m_dwSockObjNum << std::endl;
}