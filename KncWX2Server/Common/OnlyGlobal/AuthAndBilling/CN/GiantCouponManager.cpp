...
ImplToStringW( KGiantCouponManager )
{
	return stm_;
}

ImplPfID( KGiantCouponManager, PI_LOGIN_GIANT_COUPON );

ImplementSingleton( KGiantCouponManager );

KGiantCouponManager::KGiantCouponManager()
{
}
...
void KGiantCouponManager::RegToLua()
{
	lua_tinker::class_add<KGiantCouponManager>( g_pLua, "KGiantCouponManager" );
	lua_tinker::class_inh<KGiantCouponManager, KGiantBaseManager>( g_pLua );

	lua_tinker::decl( g_pLua, "GiantCouponManager", this );
}

KThread* KGiantCouponManager::CreateThread()
{
	return new KGiantBilling();
}

void KGiantCouponManager::MakeEventFromReceivedPacket()
{
...
void KGiantCouponManager::Handle_OnCommonPacket(KGiantCommonPacket& kReceivedPacket)
{
	switch( kReceivedPacket.GetCommand() )
	{
	case KGiantCommonPacket::GCP_CCT_INITIALIZE:
...
void KGiantCouponManager::Handle_OnBillingPacket(KGiantBillingPacket& kReceivedPacket)
{
	switch( kReceivedPacket.GetCommand() )
	{
	// 카드 충전 필요시 추후 구현해야함
