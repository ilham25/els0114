...
bool KBRPayletterBillingPacket::Read8Byte( __int64& iData, int& iCP )
{
...
bool KBRPayletterBillingPacket::Read( KEPL_BR_HEALTH_CHECK_ACK& kPacket )
{
	int iCP = 0;
...
bool KBRPayletterBillingPacket::Write( const KEPL_BR_HEALTH_CHECK_REQ& kPacket )
{
	int iCP = 0;
...
bool KBRPayletterBillingPacket::Read( KEPL_BR_REGCOUPON_ACK& kPacket )
{
...
bool KBRPayletterBillingPacket::Write( const KEBILL_USE_COUPON_REQ& kPacket )
{
...
// 선물하기 기능
bool KBRPayletterBillingPacket::Read( KEPL_BR_GIFTITEM_ACK& kPacket )
{
	int iCP = 0;
	kPacket.m_usReqLen = m_usReqLen;		// IN	1
	kPacket.m_usReqType = m_usReqType;		// IN	2
...
//bool KBRPayletterBillingPacket::Write( const KEPL_BR_GIFTITEM_REQ& kPacket )
//{
...
bool KBRPayletterBillingPacket::Write( const KEBILL_GIFT_ITEM_REQ& kPacket )
{
	GTX_PK_GIFTITEM tempPacket;
	m_usReqLen = tempPacket.ReqLen = sizeof(GTX_PK_GIFTITEM);
	m_usReqType = tempPacket.ReqType = GTX_PK_TYPE_GIFTITEM;
