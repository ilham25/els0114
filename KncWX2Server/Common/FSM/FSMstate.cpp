...
int FSMstate::GetOutput( int iInput ) const
{
...
bool FSMstate::CheckState( int nStateNum_, ... )
{
    va_list marker;
    va_start( marker, nStateNum_ );

    for( int i = 0; i < nStateNum_; i++ )
    {
        if( va_arg( marker, int ) == m_iStateID ) return true;
    }
