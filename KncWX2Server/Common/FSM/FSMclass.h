...
public:
	...
	virtual ~FSMclass();				// clean up memory usage
	...
	FSMstate *GetState( int iStateID ) const;	// return the FSMstate object pointer
    ...
    // florist. 
    virtual const wchar_t* GetStateIDString(int iStateID) const = 0;
    virtual const wchar_t* GetInputIDString(int iInputID) const = 0;
};
