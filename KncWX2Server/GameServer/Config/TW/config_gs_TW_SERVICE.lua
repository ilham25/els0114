...
-- Server Group 수 설정 --
SimLayer:SetServerGroupNum( 2 ) -- 서버군의 수(진입구조 개편 관련하여 필요)
...
DBLayer:AddDB( DC_PUBLISHER_BILLING,	'GASHBILLING_GAMANIA_TAIWAN_SERVICE.dsn', 5, false )
...
elseif Server:GetServerGroupID() == 1 then
	DBLayer:AddDB( DC_GAME,		'Game02_GAIA.dsn',	15, false )
	DBLayer:AddDB( DC_LOG,		'Log_GAIA.dsn',		10, false )
else
	DBLayer:AddDB( DC_GAME,		'Game01_SOLES.dsn',	15, false )
	DBLayer:AddDB( DC_LOG,		'Log_SOLES.dsn',	10, false )
end
...
then
	GASHBillingTCPManager:InitGASHBillingTCPValue( '300148', 'AF' )
	GASHBillingTCPManager:InitGASHBillingTCPAddress( '10.9.11.2', 5412, 5413 )
	GASHBillingTCPManager:SetNumThread( 1 )
end
