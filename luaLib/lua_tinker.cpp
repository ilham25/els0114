...
/* read                                                                      */ 
/*---------------------------------------------------------------------------*/ 
template<> bool lua_tinker::read_(lua_State *L, int index, std::string& ret )
{
	if( lua_isstring(L, index ) )
	{
		ret = (char*)lua_tostring(L, index);
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, char*& ret )
{
	if( lua_isstring(L, index ) )
	{
		char* pszText = (char*)lua_tostring(L, index);
		strncpy( ret, pszText, strlen( pszText ) );
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, unsigned char& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (unsigned char)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, short& ret )
{
	if( lua_isnumber( L, index ) )
	{
		ret = (short)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, unsigned short& ret )
{
	if( lua_isnumber( L, index ) )
	{
		ret = (unsigned short)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, long& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (long)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, unsigned long& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (unsigned long)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, int& ret)
{
	if( lua_isnumber( L, index ) )
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, unsigned int& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (unsigned int)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, float& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (float)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, double& ret)
{
	if( lua_isnumber( L, index ) )
	{
		ret = (double)lua_tonumber(L, index);		
		return true;
	}
	else
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, bool& ret)
{
	if( lua_isboolean(L, index) )
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, __int64& ret)
{
	if(lua_isnumber(L,index))
	{
		ret = (__int64)lua_tonumber(L, index);
		return true;
	}
	else if( lua_isuserdata(L, index) )
	{
		ret = *(__int64*)lua_touserdata(L, index);
		return true;
	}
	else 
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, unsigned __int64& ret)
{
	if(lua_isnumber(L,index))
	{
		ret = (unsigned __int64)lua_tonumber(L, index);
		return true;
	}
	else if( lua_isuserdata(L, index) )
	{
		ret = *(unsigned __int64*)lua_touserdata(L, index);
		return true;
	}
	else 
	{
...
template<>
bool lua_tinker::read_(lua_State *L, int index, lua_tinker::table& ret)
{
...
//{{ dmlee 2008.06.01
template<>	
bool lua_tinker::pop_(lua_State *L, lua_tinker::table& ret)
{
	if( lua_istable(L, lua_gettop(L) ) )
	{
		ret = table(L, lua_gettop(L));
		return true;
	}
	else
	{
