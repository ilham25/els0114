...
//{{ robobeg : 2010-09-15
  if ( iEncoding != LUA_ENCODING_LEGACY && filename )
  {
      c = getc( ctx.fp );
      if ( c == 0xef )
      {
          if ( getc( ctx.fp ) == 0xbb && getc( ctx.fp ) == 0xbf )
          {
          }
          else
          {
            lua_pushfstring(L, "invalid encoding %s", chunkname+1);
            fclose(ctx.fp);
            return LUA_ERRFILE;
          }//if.. else..
      }
      ...
      {
          if ( c == BCDUMP_HEAD1 || iEncoding == LUA_ENCODING_UNKNOWN )
              ungetc( c, ctx.fp );
          else
          {
            lua_pushfstring(L, "invalid encoding %s", chunkname+1);
