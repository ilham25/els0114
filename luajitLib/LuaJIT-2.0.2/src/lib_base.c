...
LJLIB_CF(dofile_UTF8)
{
  GCstr *fname = lj_lib_optstr(L, 1);
  setnilV(L->top);
  L->top = L->base+1;
  if (luaL_loadfilex(L, fname ? strdata(fname) : NULL, NULL, LUA_ENCODING_UTF8 ) != 0 )
    lua_error(L);
  lua_call(L, 0, LUA_MULTRET);
  return (int)(L->top - L->base) - 1;
}
...
LJLIB_CF(print)
{
  ...
  for (i = 0; i < nargs; i++) {
    ...
    if (shortcut && tvisstr(o)) {
      ...
      L->top--;
//{{ robobeg : 2008-06-14
        if ( lua_getencoding( L ) == LUA_ENCODING_UTF8 )
        {
            if ( size > 0 )
            {
                int iSize;
                iSize = MultiByteToWideChar( CP_UTF8, 0, str, ( int )size, NULL, 0 );
                if ( iSize > 0 )
                {
                    ...
                    wszBuffer = ( iSize > MAX_PATH ) ? (wchar_t*) malloc( iSize * sizeof(wchar_t) ) : &wszTemp[0];
                    iSize = MultiByteToWideChar( CP_UTF8, 0, str, ( int )size, wszBuffer, iSize );
                    iSize2 = WideCharToMultiByte( CP_ACP, 0, wszBuffer, iSize, NULL, 0, NULL, NULL );
                    if ( iSize2 > 0 )
                    {
                        char   szTemp[MAX_PATH];
                        char*  szBuffer;
                        
                        szBuffer = ( iSize2 > MAX_PATH ) ? (char*) malloc( iSize2 ) : &szTemp[0];
                        iSize2 = WideCharToMultiByte( CP_ACP, 0, wszBuffer, iSize, szBuffer, iSize2, NULL, NULL );
                        if (i)
                            putchar('\t');
                        fwrite( szBuffer, 1, iSize2, stdout );
                        if ( szBuffer != &szTemp[0] )
                            free( szBuffer );
                    }//if
                    if ( wszBuffer != &wszTemp[0] )
                        free( wszBuffer );
                }
            }//if
            continue;
