...
namespace KBenchMark_MXM
{
	...
	//
	  ...
	  //100점으로 환산...
	  float fScore = static_cast<float>(( 100.0f / 1000.0f ) * min_mflops);
	  ...
	  //int iScore = 0;
	  //if( min_mflops <= 500.0f )
		 // iScore = 0;
	  //else if( min_mflops <= 1000.0f )
		 // iScore = 1;
	  //else
		 // iScore = 2;
	...
	# define TIME_SIZE 40
	  ...
	  len = std::strftime ( time_buffer, TIME_SIZE, "%d %B %Y %I:%M:%S %p", &t );
