...
#define BCAP_VERSION			_BCAP_MAKE_VER(1,6,2,51)		// 1.6.2.51
...
public :
	...
	WCHAR* MakePathnameByDateW(WCHAR* szTargetDirectory, WCHAR* szName, WCHAR* szBuf, int nBufLen)
	{
		SYSTEMTIME	sysTime;
		GetLocalTime(&sysTime);

		wsprintf(szBuf, L"%s\\%s %d-%02d-%02d %02d-%02d-%02d.avi", szTargetDirectory, szName, sysTime.wYear, sysTime.wMonth, sysTime.wDay, sysTime.wHour, sysTime.wMinute, sysTime.wSecond);
...
inline BOOL BCapConfigPreset(BCAP_CONFIG* pConfig, BCAP_PRESET preset)
{
	...
	case BCAP_PRESET_NAVER_BLOG :
		pConfig->VideoSizeW = 504;			// 512x378
		pConfig->VideoSizeH = 0;
		pConfig->VideoFPS = 24;
		pConfig->AudioSampleRate = 44100;	// 44k
		break;
