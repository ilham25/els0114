...
#define AHNHS_CHKOPT_REALTIME_ENGINE_UPDATE		0x10000
...
#define HS_ERR_AUTH_RAWDATA_FAILED_TO_RETRIEVE	0x00000903		// AuthenticateLocalPC 에서 사용하는 Raw Data를 얻어오지 못했습니다.
...
extern "C"
{
	...
	#define _AhnHS_GetAuthRawData(arg1,arg2)				HS_ERR_OK
...
/*
 * 
 * @remarks	사용자 Local PC 인증을 위해 추출된 정보들 전달하는 함수
 *
 * @param	[IN OUT] PBYTE pbyData		: Local PC 인증을 위해 추출된 정보(Raw Data)
 * @param	[IN OUT] PDWORD pdwDataSize	: pbyData 데이터의 크기
 *
 * @retval	HS_ERR_OK : 성공, 이외는 실패
 * @remarks
 *			
...
int
__stdcall 
_AhnHS_GetAuthRawData( IN OUT PBYTE pbyData, IN OUT PDWORD pdwDataSize );
