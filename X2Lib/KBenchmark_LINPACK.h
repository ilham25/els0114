...
namespace KBenchMark_LINPACK
{
	...
	# define LDA ( N + 1 )
	  ...
	  ops = ( float ) ( 2 * N * N * N ) / 3.0f + 2.0f * ( float ) ( N * N );
	//
	...
	//
	  ...
	  total = static_cast<float>(time[0] + time[1]);
	...
	//
	  ...
	  //100점으로 환산...
	  float fScore = static_cast<float>(( 100.0f / 2500.0f ) * time[3]);
	...
	//  Compute multipliers.
	//
		t = static_cast<float>(-1.0 / a[k-1+(k-1)*lda]);
	...
	//
	{
	  ...
	  {
		r = static_cast<float>(r / 2.0E+00);
	  }

	  return static_cast<float>( 2.0E+00 * r );
	}
	//****************************************************************************80
	...
	//
	{
	  ...
	  for ( j = 1; j <= n; j++ )
	  {
		for ( i = 1; i <= n; i++ )
		{
		  a[i-1+(j-1)*lda] = static_cast<float>(r4_random ( init ) - 0.5);
		}
	  }
