...
void CX2OfflineEnchantTable::EnsureLoaded()
{
	...
	kInfo = g_pKTDXApp->GetDeviceManager()->GetMassFileManager()->LoadDataFile( SCRIPT_ENCHANT_TABLE, true, true );
#else
	...
	bool bRan	= g_pKTDXApp->LoadAndDoMemory( g_pKTDXApp->GetLuaBinder(), SCRIPT_ENCHANT_TABLE );
	bool bAdded	= ( m_iProbRows > 0 );
	//}}

	if( false == bRan || false == bAdded )
	{
		bRan	= g_pKTDXApp->LoadAndDoMemory( g_pKTDXApp->GetLuaBinder(), SCRIPT_ENCHANT_TABLE, false );
