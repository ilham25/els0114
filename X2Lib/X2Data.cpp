...
//{{ Iruha : 2026-09-05 // AI party phase 0 - probe whether this client's data
//            actually carries the PvP bot NPCs the AI-party feature is built
//            on. The enum existing proves nothing: the templet, its state
//            machine .lua and its stat row all come out of the .kom archives,
//            which this repo does not contain.
//
//            Deliberately NOT behind its own short-lived *_DEBUG flag - the
//            whole AI-party feature rides SERV_IRUHADEV_OFFLINE. What makes it
//            removable is that it is ONE contiguous block in ONE function with
//            every line tagged AIPARTY. Phase 5 deletes it; keep it that way.
//            See AI_PARTY_PLAN.md phase 0.
#ifdef SERV_IRUHADEV_OFFLINE
	{
		struct ProbeEntry
		{
			CX2UnitManager::NPC_UNIT_ID		m_eID;
			const wchar_t*					m_szName;
			bool							m_bPrimary;		///< one of the six NUI_PVP_BOT_*
		};

		static const ProbeEntry s_aProbe[] =
		{
			{ CX2UnitManager::NUI_PVP_BOT_ELSWORD,               L"NUI_PVP_BOT_ELSWORD",               true },
			{ CX2UnitManager::NUI_PVP_BOT_AISHA,                 L"NUI_PVP_BOT_AISHA",                 true },
			{ CX2UnitManager::NUI_PVP_BOT_RENA,                  L"NUI_PVP_BOT_RENA",                  true },
			{ CX2UnitManager::NUI_PVP_BOT_RAVEN,                 L"NUI_PVP_BOT_RAVEN",                 true },
			{ CX2UnitManager::NUI_PVP_BOT_EVE,                   L"NUI_PVP_BOT_EVE",                   true },
			{ CX2UnitManager::NUI_PVP_BOT_CHUNG,                 L"NUI_PVP_BOT_CHUNG",                 true },
			{ CX2UnitManager::NUI_PVP_HERO_LOW,                  L"NUI_PVP_HERO_LOW",                  false },
			{ CX2UnitManager::NUI_PVP_HERO_PENENSIO,             L"NUI_PVP_HERO_PENENSIO",             false },
			{ CX2UnitManager::NUI_PVP_HERO_NOA,                  L"NUI_PVP_HERO_NOA",                  false },
			{ CX2UnitManager::NUI_PVP_HERO_SPIKA,                L"NUI_PVP_HERO_SPIKA",                false },
			{ CX2UnitManager::NUI_PVP_HERO_LIME,                 L"NUI_PVP_HERO_LIME",                 false },
			{ CX2UnitManager::NUI_PVP_HERO_AMELIA,               L"NUI_PVP_HERO_AMELIA",               false },
			{ CX2UnitManager::NUI_PVP_HERO_EDAN,                 L"NUI_PVP_HERO_EDAN",                 false },
			{ CX2UnitManager::NUI_PVP_HERO_BALAK,                L"NUI_PVP_HERO_BALAK",                false },
			{ CX2UnitManager::NUI_PVP_HERO_CODE_Q_PROTO_00,      L"NUI_PVP_HERO_CODE_Q_PROTO_00",      false },
			{ CX2UnitManager::NUI_PVP_HERO_APPLE,                L"NUI_PVP_HERO_APPLE",                false },
			{ CX2UnitManager::NUI_PVP_RUNE_GUARD,                L"NUI_PVP_RUNE_GUARD",                false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_LOW,              L"NUI_CSM_PVP_HERO_LOW",              false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_PENENSIO,         L"NUI_CSM_PVP_HERO_PENENSIO",         false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_NOA,              L"NUI_CSM_PVP_HERO_NOA",              false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_SPIKA,            L"NUI_CSM_PVP_HERO_SPIKA",            false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_LIME,             L"NUI_CSM_PVP_HERO_LIME",             false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_AMELIA,           L"NUI_CSM_PVP_HERO_AMELIA",           false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_EDAN,             L"NUI_CSM_PVP_HERO_EDAN",             false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_BALAK,            L"NUI_CSM_PVP_HERO_BALAK",            false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_CODE_Q_PROTO_00,  L"NUI_CSM_PVP_HERO_CODE_Q_PROTO_00",  false },
			{ CX2UnitManager::NUI_CSM_PVP_HERO_APPLE,            L"NUI_CSM_PVP_HERO_APPLE",            false },
		};

		// Normally CX2OfflineServer::Instance() opens the log, but this runs
		// from CX2StateStartUp and may get here first. Open() is idempotent and
		// truncates only on the first call, so either order is correct.
		CX2OfflineLog::Open();

		int iPrimaryTotal = 0, iPrimaryOK = 0, iFallbackTotal = 0, iFallbackOK = 0;

		for( int i = 0; i < (int)( sizeof( s_aProbe ) / sizeof( s_aProbe[0] ) ); ++i )
		{
			const ProbeEntry& kEntry = s_aProbe[i];

			if( true == kEntry.m_bPrimary )		++iPrimaryTotal;
			else								++iFallbackTotal;

			// GetNPCUnitInfo() is the same call the CX2GUNPC creation path
			// makes: it looks the templet up and lazily loads that NPC's own
			// .lua. NULL means there is no templet row at all; a templet whose
			// init data came back empty means the .lua is not in the .kom.
			const CX2UnitManager::NPCUnitInfo* pInfo = m_pUnitManager->GetNPCUnitInfo( kEntry.m_eID );

			if( NULL == pInfo )
			{
				CX2OfflineLog::Server( L"AIPARTY  %-32s id=%-6d NO_TEMPLET", kEntry.m_szName, (int)kEntry.m_eID );
				continue;
			}

			int iStates = -1;
#ifdef X2OPTIMIZE_NPC_NONHOST_SIMULATION
			if( NULL != pInfo->m_pInitData )
				iStates = (int)pInfo->m_pInitData->m_init.m_state.m_vecStateData.size();
#endif X2OPTIMIZE_NPC_NONHOST_SIMULATION

			int iSkinMesh = -1;
			if( NULL != pInfo->m_pInitData )
				iSkinMesh = (int)pInfo->m_pInitData->m_init.m_device.m_vecstrReadyXSkinMesh.size();

			const bool bHasStat = ( pInfo->m_stat.m_nNPCUnitID == kEntry.m_eID );

			const wchar_t* szVerdict = L"OK";
			bool bUsable = true;
			if( true == pInfo->m_templet.m_LuaFileName.empty() )
			{
				szVerdict = L"NO_LUA_FILE";
				bUsable = false;
			}
			else if( 0 >= iStates && 0 >= iSkinMesh )
			{
				szVerdict = L"LUA_NOT_LOADED";
				bUsable = false;
			}

			if( true == bUsable )
			{
				if( true == kEntry.m_bPrimary )		++iPrimaryOK;
				else								++iFallbackOK;
			}

			CX2OfflineLog::Server( L"AIPARTY  %-32s id=%-6d %-14s lua=\"%s\" name=\"%s\" isNpc=%d classType=%d stat=%s hp=%.0f atkP=%.0f atkM=%.0f defP=%.0f defM=%.0f skinMesh=%d states=%d",
									kEntry.m_szName,
									(int)kEntry.m_eID,
									szVerdict,
									pInfo->m_templet.m_LuaFileName.c_str(),
									pInfo->m_templet.m_Name.c_str(),
									(int)pInfo->m_templet.m_bIsNPC,
									(int)pInfo->m_templet.m_ClassType,
									( true == bHasStat ) ? L"YES" : L"NO",
									pInfo->m_stat.m_fMaxHP,
									pInfo->m_stat.m_fAtkPhysic,
									pInfo->m_stat.m_fAtkMagic,
									pInfo->m_stat.m_fDefPhysic,
									pInfo->m_stat.m_fDefMagic,
									iSkinMesh,
									iStates );
		}//for

		CX2OfflineLog::Server( L"AIPARTY  probe complete: NUI_PVP_BOT_* usable %d/%d, fallback NPCs usable %d/%d",
								iPrimaryOK, iPrimaryTotal, iFallbackOK, iFallbackTotal );
	}
#endif SERV_IRUHADEV_OFFLINE
//}}
