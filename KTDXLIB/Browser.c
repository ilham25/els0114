...
// dmlee 2008.03.27
// 브라우저안에서 브라우저종료 event가 발생한 경우에 browser control의 부모 윈도우까지 닫아주기 위해서 IDispatch::Invoke 구현했음.
// GetExternal에서 IDispatch 포인터 넘겨주도록 했는데 GetExternal함수 호출 안되는 듯. 
// 그리고 IDispatch::Invoke 함수 안에서 dispIdMember가 이상한 값이 들어옴. 
// 뭔가 잘못됐는데 뭔지 모르겠음. 일단 IDispatch 관련 코드는 모두 comment out
// 일단 꽁수로 브라우저가 닫힐때 UI_Release 함수 호출되는 것을 이용, 거기에서 그냥 부모 윈도우를 꺼줌


// dmlee 2008.04.07 
// 넥슨캐시충전페이지에서 게임문화상품권 번호입력 혹은 주민번호 입력시 자동으로 다음 edit box control로 focus 이동 가능하도록 keyup event 처리되게 수정



//{{ dmlee 2008.03.27 IDispatch 관련 
#include <exdispid.h>
//}} dmlee 2008.03.27 IDispatch 관련 
...
HRESULT STDMETHODCALLTYPE Dispatch_Invoke( IDispatch * This, DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, 
									VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr);

IDispatchVtbl MyIDispatchTable = 
{
...
{
	//{{ dmlee 2008.03.28
	// note!! 일단 꽁수로 여기에서 browser control을 가지고 있는 최상위 dialog window를 닫는다
	_IOleInPlaceSiteEx* _iOleInPlaceSiteEx = NULL;
	UI_QueryInterface( (IDocHostUIHandler*) This, &IID_IOleInPlaceSite, (LPVOID*) &_iOleInPlaceSiteEx );
	SendMessage( _iOleInPlaceSiteEx->frame.rootwindow, WM_CLOSE, 0, 0 );
	//}} dmlee 2008.03.28
...
HRESULT STDMETHODCALLTYPE UI_TranslateAccelerator(IDocHostUIHandler FAR* This, LPMSG lpMsg, const GUID __RPC_FAR *pguidCmdGroup, DWORD nCmdID)
{
	...
	//{{ dmlee 2008.04.07 - 넥슨캐시충전페이지에서 주민번호입력시 다음 editbox로 자동이동할 수 있도록
	if( lpMsg && lpMsg->message == WM_KEYUP )
	{
		return (S_FALSE);
	}
	//}} dmlee 2008.04.07 - 넥슨캐시충전페이지에서 주민번호입력시 다음 editbox로 자동이동할 수 있도록
...
HRESULT STDMETHODCALLTYPE UI_GetExternal(IDocHostUIHandler FAR* This, IDispatch __RPC_FAR *__RPC_FAR *ppDispatch)
{
	...
	//{{ dmlee 2008.03.27 IDispatch 관련
	//UI_QueryInterface( (IDocHostUIHandler *) This, &IID_IDispatch, ppDispatch );
	//return(S_OK);
	//}} dmlee 2008.03.27 IDispatch 관련
...
HRESULT STDMETHODCALLTYPE Site_QueryInterface(IOleClientSite FAR* This, REFIID riid, void ** ppvObject)
{
	...
	//{
	//	*ppvObject = &((_IOleClientSiteEx *)This)->ui.idispatch;
	//}
	//}} dmlee 2008.03.27 IDispatch 관련 
										  ...
										  VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	case DISPID_WINDOWCLOSING:
		{
			// 여기에서 부모 윈도우를 닫아준다
			// m_pWebContainer->OnWindowClosing (pDispParams->rgvarg[1].boolVal, pDispParams->rgvarg[0].pboolVal);
			// return S_OK;
		} break;
	}
...
long DisplayHTMLStr(HWND hwnd, const char* string)
{
	IWebBrowser2	*webBrowser2;
...
badalloc:	webBrowser2->lpVtbl->Release(webBrowser2);
		...
		// Call the Navigate2() function to actually display the page.
		if( NULL == strPostString_ || strlen(strPostString_) == 0 )
			webBrowser2->lpVtbl->Navigate2(webBrowser2, &myURL, 0, 0, 0, 0);
		else
		{
			// Post 방식으로 던짐.
			OLECHAR cheader[MAX_PATH];
			SAFEARRAY FAR *psaPostData;
			VARIANT vaPostHeader;
			VARIANT vaPost;
			HRESULT hr;

#ifdef	CONVERSION_VS
			wcscpy_s( cheader, _countof(cheader), L"Content-Type: application/x-www-form-urlencoded\r\n" );
#else	CONVERSION_VS
			wcscpy( cheader, L"Content-Type: application/x-www-form-urlencoded\r\n" );
#endif	CONVERSION_VS

			nByte = strlen(strPostString_);			
			vaPostHeader.vt = VT_BSTR;
			vaPostHeader.bstrVal = SysAllocString( cheader );

			{
				SAFEARRAYBOUND sab;
				long i;

				sab.lLbound = 0; sab.cElements = nByte;
				psaPostData = SafeArrayCreate(VT_UI1, 1, &sab);

				for( i = 0; i < (signed)sab.cElements; i++ )					
					SafeArrayPutElement( psaPostData, &i, (void*)(strPostString_ + i) );
			}

			vaPost.vt = VT_ARRAY;
			vaPost.parray = psaPostData;
			hr = webBrowser2->lpVtbl->Navigate2(webBrowser2, &myURL, 0, 0, &vaPost, &vaPostHeader );
			VariantClear(&vaPost);
...
// @hRootWnd : browser control의 최상위 부모 dialog window
long EmbedBrowserObject( HWND hwnd, HWND hRootWnd )
{
	...
	//{{ dmlee 2008.03.27 IDispatch 관련 
	//_iOleClientSiteEx->ui.idispatch.idispatch.lpVtbl = &MyIDispatchTable;
	//}} dmlee 2008.03.27 IDispatch 관련 

	//{{ dmlee 2008.03.28
	_iOleClientSiteEx->inplace.frame.rootwindow = hRootWnd;
	//}} dmlee 2008.03.28
...
{	
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		webBrowser2->lpVtbl->get_Application( webBrowser2, &lpDisp );

		if( lpDisp != NULL ) 
		{
			HRESULT hr = lpDisp->lpVtbl->QueryInterface(lpDisp, &IID_IOleInPlaceActiveObject, (LPVOID*)&pIOIPAO);
			if( SUCCEEDED(hr) ) 
			{
