...
#endif  X2OPTIMIZE_BANDI_DYNAMICVB
{
	...
	if(!GetTextureFormat(g_pD3D, m_fmt_d3d, m_fmt_bvl, m_pixel_size))
		ASSERT(0);
#else   X2OPTIMIZE_BANDI_DYNAMICVB
...
#ifdef  X2OPTIMIZE_BANDI_DYNAMICVB
    LPDIRECT3DDEVICE9 d3d9_device = g_pd3dDevice; 
    if ( d3d9_device == NULL )
        return E_FAIL;
#else   X2OPTIMIZE_BANDI_DYNAMICVB
...
BYTE*	CBandiVideoTexture_DX9::Lock(INT &pitch)
{
	...
	BYTE *buf = NULL;
	if ( NULL != m_texture )
#else // FIX_REFORM_ENTRY_POINT_3RD		// 김종훈, 진입 구조 개편 3차 ( 크래시 ) 수정
	BYTE *buf = NULL;
...
HRESULT	CBandiVideoTexture_DX9::Draw(INT x, INT y, INT width, INT height)
{
	...
	hr = CKTDGStateManager::SetTexture(0, m_texture);
    ...
    KD3DBEGIN()
	    ...
	    CKTDGStateManager::PushSamplerState( 0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
	    CKTDGStateManager::PushSamplerState( 0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
