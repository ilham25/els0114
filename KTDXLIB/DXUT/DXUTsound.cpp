                                   ...
                                   DWORD dwCoopLevel )
{
    ...
    if( FAILED( hr = DirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
        return E_FAIL;//DXUT_ERR( L"DirectSoundCreate8", hr );

    // Set DirectSound coop level
    if( FAILED( hr = m_pDS->SetCooperativeLevel( hWnd, dwCoopLevel ) ) )
        return E_FAIL;//DXUT_ERR( L"SetCooperativeLevel", hr );
                                               ...
                                               DWORD dwPrimaryBitRate )
{
    ...
    if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &pDSBPrimary, NULL ) ) )
        return E_FAIL;//DXUT_ERR( L"CreateSoundBuffer", hr );
    ...
    if( FAILED( hr = pDSBPrimary->SetFormat(&wfx) ) )
        return E_FAIL;//DXUT_ERR( L"SetFormat", hr );
...
HRESULT CSoundManager::Get3DListenerInterface( LPDIRECTSOUND3DLISTENER* ppDSListener )
{
    ...
    *ppDSListener = NULL;
    ...
    {
        SAFE_RELEASE( pDSBPrimary );
        return E_FAIL;//DXUT_ERR( L"QueryInterface", hr );
    }
                               ...
                               DWORD dwNumBuffers )
{
	...
	{
		ErrorLogMsg( KEM_ERROR124, L"DS_NO_VIRTUALIZATION" );
    ...
    if( FAILED(hr) )
    {
		...
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_ALLOCATED" );
		}
		else if ( hr == DSERR_BADFORMAT )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_BADFORMAT" );
		}
		else if ( hr == DSERR_BUFFERTOOSMALL )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_BUFFERTOOSMALL" );
		}
		else if ( hr == DSERR_CONTROLUNAVAIL )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_CONTROLUNAVAIL" );
		}
		else if ( hr == DSERR_DS8_REQUIRED )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_DS8_REQUIRED" );
		}
		else if ( hr == DSERR_INVALIDCALL )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_INVALIDCALL" );
		}
		else if ( hr == DSERR_INVALIDPARAM )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_INVALIDPARAM" );
		}
		else if ( hr == DSERR_NOAGGREGATION )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_NOAGGREGATION" );
		}
		else if ( hr == DSERR_OUTOFMEMORY )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_OUTOFMEMORY" );
		}
		else if ( hr == DSERR_UNINITIALIZED )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_UNINITIALIZED" );
		}
		else if ( hr == DSERR_UNSUPPORTED )
		{
			ErrorLogMsg( KEM_ERROR124, L"DSERR_UNSUPPORTED" );
		}
        ...
        //E_FAIL;//DXUT_ERR( L"CreateSoundBuffer", hr );
    ...
    if( (dwCreationFlags & DSBCAPS_CTRLFX) == 0 )
    {
        for( i=1; i<dwNumBuffers; i++ )
        {
            ...
            {
                //E_FAIL;//DXUT_ERR( L"DuplicateSoundBuffer", hr );
...
LFail:
 	ErrorLogMsg( KEM_ERROR126, strWaveFileName );
    // Cleanup
							  ...
							  DWORD dwNumBuffers )
{
	...
	pWaveFile->Open( pbData, ulDataSize, NULL, WAVEFILE_READ );
                                        ...
                                        HANDLE hNotifyEvent )
{
                                                          ...
                                                          aPosNotify ) ) )
    {
        ...
        SAFE_DELETE_ARRAY( aPosNotify );
        return E_FAIL;//DXUT_ERR( L"SetNotificationPositions", hr );
    }
...
HRESULT CSound::FillBufferWithSound( LPDIRECTSOUNDBUFFER pDSB, BOOL bRepeatWavIfBufferLarger )
{
    ...
    if( FAILED( hr = RestoreBuffer( pDSB, NULL ) ) )
        return E_FAIL;//DXUT_ERR( L"RestoreBuffer", hr );

    // Lock the buffer down
    if( FAILED( hr = pDSB->Lock( 0, m_dwDSBufferSize,
                                 &pDSLockedBuffer, &dwDSLockedBufferSize,
                                 NULL, NULL, 0L ) ) )
        return E_FAIL;//DXUT_ERR( L"Lock", hr );
    ...
    if( FAILED( hr = m_pWaveFile->Read( (BYTE*) pDSLockedBuffer,
                                        ...
                                        &dwWavDataRead ) ) )
        return E_FAIL;//DXUT_ERR( L"Read", hr );
        ...
        if( bRepeatWavIfBufferLarger )
        {
            ...
            while( dwReadSoFar < dwDSLockedBufferSize )
            {
                ...
                if( FAILED( hr = m_pWaveFile->ResetFile() ) )
                    return E_FAIL;//DXUT_ERR( L"ResetFile", hr );
...
HRESULT CSound::RestoreBuffer( LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored )
{
    ...
    if( FAILED( hr = pDSB->GetStatus( &dwStatus ) ) )
        return E_FAIL;//DXUT_ERR( L"GetStatus", hr );
...
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, LONG lVolume, LONG lFrequency, LONG lPan )
{
    ...
    if( pDSB == NULL )
        return E_FAIL;//DXUT_ERR( L"GetFreeBuffer", E_FAIL );
    ...
    if( bRestored )
    {
        ...
        if( FAILED( hr = FillBufferWithSound( pDSB, FALSE ) ) )
            return E_FAIL;//DXUT_ERR( L"FillBufferWithSound", hr );
    }
...
HRESULT CStreamingSound::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
    ...
    if( FAILED( hr = m_apDSBuffer[0]->GetCurrentPosition( &dwCurrentPlayPos, NULL ) ) )
        return E_FAIL;//DXUT_ERR( L"GetCurrentPosition", hr );
...
HRESULT CWaveFile::Open( LPWSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            if( NULL == ( hResInfo = FindResource( NULL, strFileName, L"WAVE" ) ) )
            {
                if( NULL == ( hResInfo = FindResource( NULL, strFileName, L"WAV" ) ) )
                    return E_FAIL;//DXUT_ERR( L"FindResource", E_FAIL );
            }

            if( NULL == ( hResData = LoadResource( GetModuleHandle(NULL), hResInfo ) ) )
                return E_FAIL;//DXUT_ERR( L"LoadResource", E_FAIL );

            if( 0 == ( dwSize = SizeofResource( GetModuleHandle(NULL), hResInfo ) ) )
                return E_FAIL;//DXUT_ERR( L"SizeofResource", E_FAIL );

            if( NULL == ( pvRes = LockResource( hResData ) ) )
                return E_FAIL;//DXUT_ERR( L"LockResource", E_FAIL );

            m_pResourceBuffer = new CHAR[ dwSize ];
            if( m_pResourceBuffer == NULL )
                return E_FAIL;//DXUT_ERR( L"new", E_OUTOFMEMORY );
        ...
        if( FAILED( hr = ReadMMIO() ) )
        {
            ...
            mmioClose( m_hmmio, 0 );
            return E_FAIL;//DXUT_ERR( L"ReadMMIO", hr );
        }
    ...
    else
    {
        m_hmmio = mmioOpen( strFileName, NULL, MMIO_ALLOCBUF  |
                                                  ...
                                                  MMIO_CREATE );
        if( NULL == m_hmmio )
            return E_FAIL;//DXUT_ERR( L"mmioOpen", E_FAIL );

        if( FAILED( hr = WriteMMIO( pwfx ) ) )
        {
            mmioClose( m_hmmio, 0 );
            return E_FAIL;//DXUT_ERR( L"WriteMMIO", hr );
        }
...
HRESULT CWaveFile::Open( BYTE* data, ULONG size, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
	HRESULT hr;
	...
	//윈도우 98에서는 유니코드 mmioOpen을 지원하지 않으므로 명시적으로 A를 선언한다
	m_hmmio = mmioOpenA( NULL, &mmioInfo, MMIO_ALLOCBUF | MMIO_READ );
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    if( ( 0 != mmioDescend( m_hmmio, &m_ckRiff, NULL, 0 ) ) )
        return E_FAIL;//DXUT_ERR( L"mmioDescend", E_FAIL );

    // Check to make sure this is a valid wave file
    if( (m_ckRiff.ckid != FOURCC_RIFF) ||
        (m_ckRiff.fccType != mmioFOURCC('W', 'A', 'V', 'E') ) )
        return E_FAIL;//DXUT_ERR( L"mmioFOURCC", E_FAIL );
    ...
    // if there are extra parameters at the end, we'll ignore them
       if( ckIn.cksize < (LONG) sizeof(PCMWAVEFORMAT) )
           return E_FAIL;//DXUT_ERR( L"sizeof(PCMWAVEFORMAT)", E_FAIL );

    // Read the 'fmt ' chunk into <pcmWaveFormat>.
    if( mmioRead( m_hmmio, (HPSTR) &pcmWaveFormat,
                  sizeof(pcmWaveFormat)) != sizeof(pcmWaveFormat) )
        return E_FAIL;//DXUT_ERR( L"mmioRead", E_FAIL );
    ...
    if( pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM )
    {
        ...
        if( NULL == m_pwfx )
            return E_FAIL;//DXUT_ERR( L"m_pwfx", E_FAIL );
    ...
    else
    {
        ...
        if( NULL == m_pwfx )
            return E_FAIL;//DXUT_ERR( L"new", E_FAIL );
    ...
    {
        SAFE_DELETE( m_pwfx );
        return E_FAIL;//DXUT_ERR( L"mmioAscend", E_FAIL );
    }
    ...
    else
    {
        ...
        if( m_dwFlags == WAVEFILE_READ )
        {
            ...
            if( -1 == mmioSeek( m_hmmio, m_ckRiff.dwDataOffset + sizeof(FOURCC),
                            SEEK_SET ) )
                return E_FAIL;//DXUT_ERR( L"mmioSeek", E_FAIL );
        ...
        else
        {
            ...
            if( 0 != mmioCreateChunk( m_hmmio, &m_ck, 0 ) )
                return E_FAIL;//DXUT_ERR( L"mmioCreateChunk", E_FAIL );

            if( 0 != mmioGetInfo( m_hmmio, &m_mmioinfoOut, 0 ) )
                return E_FAIL;//DXUT_ERR( L"mmioGetInfo", E_FAIL );
        }
    }
...
HRESULT CWaveFile::Read( BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead )
{
    ...
    else
    {
        ...
        for( DWORD cT = 0; cT < cbDataIn; cT++ )
        {
            ...
            if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
            {
                if( 0 != mmioAdvance( m_hmmio, &mmioinfoIn, MMIO_READ ) )
                    return E_FAIL;//DXUT_ERR( L"mmioAdvance", E_FAIL );

                if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
                    return E_FAIL;//DXUT_ERR( L"mmioinfoIn.pchNext", E_FAIL );
            }
        ...
        if( 0 != mmioSetInfo( m_hmmio, &mmioinfoIn, 0 ) )
            return E_FAIL;//DXUT_ERR( L"mmioSetInfo", E_FAIL );
...
HRESULT CWaveFile::WriteMMIO( WAVEFORMATEX *pwfxDest )
{
    ...
    if( pwfxDest->wFormatTag == WAVE_FORMAT_PCM )
    {
        if( mmioWrite( m_hmmio, (HPSTR) pwfxDest,
                       sizeof(PCMWAVEFORMAT)) != sizeof(PCMWAVEFORMAT))
            return E_FAIL;//DXUT_ERR( L"mmioWrite", E_FAIL );
    }
    else
    {
