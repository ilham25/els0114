...
HRESULT CDXUTResourceCache::CreateTextureFromFileEx( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE9 *ppTexture )
{
	...
	ZeroMemory( &NewEntry, sizeof(DXUTCache_Texture) );
...
HRESULT CDXUTResourceCache::CreateEffectFromFile( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
	...
	ZeroMemory( &NewEntry, sizeof(DXUTCache_Effect) );
...
VOID CBaseCamera::SetViewParams( D3DXVECTOR3* pvEyePt, D3DXVECTOR3* pvLookatPt, D3DXVECTOR3* pvUpVec )
{
	...
	// oasis907 : ±è»óÀ± [2010.11.26] // Roll Angle °è»ê
	D3DXVECTOR3* pUpBasis		= (D3DXVECTOR3*) &mInvView._21;
	D3DXVECTOR3* pRightBasis	= (D3DXVECTOR3*) &mInvView._11;

	m_fCameraRollAngle = -(atan2f( pUpBasis->y, pRightBasis->y ) - D3DX_PI/2);
...
LRESULT CBaseCamera::HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_KEYUP:
        {
            ...
            D3DUtil_CameraKeys mappedKey = MapKey( (UINT)wParam );
#ifdef KEYFRAME_CAMERA
            if( mappedKey != CAM_UNKNOWN && (DWORD)mappedKey < 11 )
#else
...
#ifdef KEYFRAME_CAMERA
		if( IsKeyDown(m_aKeys[CAM_ROTATE_LEFT]) )
			m_fRollDirection = 1.f;
		else if( IsKeyDown(m_aKeys[CAM_ROTATE_RIGHT]) )
			m_fRollDirection = -1.f;
		else
			m_fRollDirection = 0.f;

		if( IsKeyDown(m_aKeys[CAM_ROTATE_CENTER]) )
			m_bRollReset = true;
		else
			m_bRollReset = false;
#endif KEYFRAME_CAMERA
...
#ifdef KEYFRAME_CAMERA	
		case 'Z':      return CAM_ROTATE_LEFT;
		case 'C':      return CAM_ROTATE_RIGHT;
		case 'X':      return CAM_ROTATE_CENTER;
#endif KEYFRAME_CAMERA	
	}
...
#ifdef KEYFRAME_CAMERA
	if(m_fRollDirection != 0)
	{
		m_fCameraRollAngle += (m_fRollDirection * 0.01f);
	}
	if(m_bRollReset)
	{
		m_fCameraRollAngle = 0;
	}
#endif KEYFRAME_CAMERA
...
#ifdef KEYFRAME_CAMERA
    D3DXMatrixRotationYawPitchRoll( &mCameraRot, m_fCameraYawAngle, m_fCameraPitchAngle, m_fCameraRollAngle);
#else
