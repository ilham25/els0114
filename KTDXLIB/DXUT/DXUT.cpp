...
//bool	g_bIsDXUTDeveloper = false;
//bool	g_bIsDXUTSpecialUser = false;
...
#ifdef CHANGE_KEY_DEVICE
	//{{ robobeg : 2008-03-27
	GET_SET_ACCESSOR( WORD, VirtualKey );
	GET_SET_ACCESSOR( DWORD, LastPostMessageTime );
	//}} robobeg : 2008-03-27
#endif
...
void DXUTSetWindow4Tool(HWND hWnd, LPDIRECT3D9 pd3d, LPDIRECT3DDEVICE9 pd3dDevice)
{
	...
	DXUTDeviceSettings *deviceSettings = new DXUTDeviceSettings;
	ZeroMemory( deviceSettings, sizeof(DXUTDeviceSettings) );
	//deviceSettings->pp.AdapterOrdinal      = D3DADAPTER_DEFAULT;
	...
	deviceSettings->pp.BackBufferWidth  = nClientWidth;
	deviceSettings->pp.BackBufferHeight = nClientHeight;
	deviceSettings->pp.Flags = D3DPRESENTFLAG_LOCKABLE_BACKBUFFER;
	deviceSettings->DeviceType = D3DDEVTYPE_HAL;
	deviceSettings->BehaviorFlags = D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED;
	...
	GetDXUTState().SetCurrentDeviceSettings( deviceSettings );
	...
	GetDXUTState().SetOverrideWidth(nClientWidth);
	GetDXUTState().SetOverrideHeight(nClientHeight);
}
...
#ifdef DXUT_KTDXLIB_SAFE_ON_RESET
HRESULT DXUTReset3DEnvironmentWithoutLost( bool bCallLostWhenFailed );
#endif
...
void DXUTParseCommandLine()
{
    ...
    int nNumArgs = 0;
    WCHAR** pstrArgList = NULL;// = CommandLineToArgvW( GetCommandLine(), &nNumArgs );
                          ...
                          HICON hIcon, HMENU hMenu, int x, int y )
{
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
		...
		*/
		...
		wndClass.cbWndExtra = 0;
		wndClass.hInstance = hInstance;
		wndClass.hIcon = hIcon; //LoadIcon (NULL, IDC_ARROW);
		wndClass.hCursor = LoadCursor (NULL, IDC_ARROW);
		wndClass.hbrBackground = (HBRUSH) GetStockObject (BLACK_BRUSH);
		wndClass.lpszMenuName =	NULL;
		...
		hWnd = CreateWindowEx( WS_EX_ACCEPTFILES, L"Direct3DWindowClass", strWindowTitle, WS_OVERLAPPEDWINDOW,
		...
		hWnd = CreateWindowEx( WS_EX_ACCEPTFILES, L"Direct3DWindowClass", strWindowTitle, WS_OVERLAPPED|WS_CAPTION,
			x, y, (rc.right-rc.left), (rc.bottom-rc.top), 0,
			hMenu, hInstance, 0 );
                          ...
                          void* pUserContext )
{
    ...
    if( GetDXUTState().GetOverrideForceVsync() == 0 )
    {
		...
		GetDXUTState().SetOverrideForceVsync( D3DPRESENT_INTERVAL_IMMEDIATE );
#endif
    }
...
#ifdef VERTICAL_SYNC_OPTION
		GetDXUTState().SetOverrideForceVsync( D3DPRESENT_INTERVAL_DEFAULT );
#endif
    }
	...
	deviceSettings.BehaviorFlags |= D3DCREATE_MULTITHREADED;
...
/*
void ShowFullScreen( HWND hwnd, int nWidth, int nHeight, bool bFullScreen )
{
	// 그래픽 모드 설정
	DEVMODE devmode;
	...
	// 변환
	LONG lRet;
	if( bFullScreen == true )	lRet = ChangeDisplaySettings( &devmode, CDS_FULLSCREEN );
	else						lRet = ChangeDisplaySettings( NULL, 0 );
	...
	if( lRet != DISP_CHANGE_SUCCESSFUL )
	{
		...
		case DISP_CHANGE_BADFLAGS :	
			lstrcpy( pMsg, L"DISP_CHANGE_BADFLAGS : 유효하지 않은 플래그" );	break;
		case DISP_CHANGE_BADMODE :	
			lstrcpy( pMsg, L"DISP_CHANGE_BADMODE : 지원하지 않는 모드" );		break;
		case DISP_CHANGE_BADPARAM :	
			lstrcpy( pMsg, L"DISP_CHANGE_BADPARAM : 유효하지 않은 인자 사용" );	break;
		case DISP_CHANGE_FAILED :	
			lstrcpy( pMsg, L"DISP_CHANGE_FAILED : 모드 변환 실패" );			break;
		case DISP_CHANGE_NOTUPDATED : 
			lstrcpy( pMsg, L"DISP_CHANGE_NOTUPDATED : 레지스트리 기록 실패" );	break;
		case DISP_CHANGE_RESTART :	
			lstrcpy( pMsg, L"DISP_CHANGE_RESTART : 재부팅 필요" );			break;
			//		case DISP_CHANGE_BADDUALVIEW :	
			lstrcpy( pMsg, L"DISP_CHANGE_BADDUALVIEW : 듀얼 뷰 실패" );		break;
		}

		MessageBox( hwnd, pMsg, L"에러 결과", MB_ICONERROR );
	}
}
*/
//--------------------------------------------------------------------------------------
...
HRESULT DXUTToggleFullScreen()
{
	...
	// 해상도 설정을 1024/768 로 하기위해
	//ShowFullScreen( DXUTGetHWND(), 1024, 768, DXUTGetDeviceSettings().pp.Windowed );
...
//{{ JHKang / 강정훈 / 2010.09.2 / 백 버퍼 재설정
#ifdef TOGGLE_WINDOW_FULLSCREEN
HRESULT DXUTRestorBackBuffer(UINT uiWidth, UINT uiHeight)
{
	HRESULT hr;
...
#ifdef FIX_ASPECT_RATIO_HACKING
			{
				RECT rcClient, rcWind;
				GetClientRect( DXUTGetHWNDDeviceWindowed(), &rcClient );
				GetWindowRect( DXUTGetHWNDDeviceWindowed(), &rcWind );

				POINT ptDiff;
				ptDiff.x = ( rcWind.right - rcWind.left ) - rcClient.right;
				ptDiff.y = ( rcWind.bottom - rcWind.top ) - rcClient.bottom;

				rcClient.right = nWindowWidth - ptDiff.x;
				rcClient.bottom = nWindowHeight - ptDiff.y;

				float fAspectRatio = (float)rcClient.right / (float)rcClient.bottom;
				if( fAspectRatio > 1.7778f )
				{
					rcClient.right = (LONG)( rcClient.bottom * 1.7778f );
					nWindowWidth = rcClient.right + ptDiff.x;
				}
			}
#endif //FIX_ASPECT_RATIO_HACKING
...
HRESULT DXUTCreate3DEnvironment( IDirect3DDevice9* pd3dDeviceFromApp )
{
	...
	/*	hr = pD3D->CreateDevice( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, 
								DXUTGetHWNDFocus(), D3DCREATE_HARDWARE_VERTEXPROCESSING,
...
#ifdef DXUT_KTDXLIB_SAFE_ON_RESET
	return DXUTReset3DEnvironmentWithoutLost( true );
#else
    hr = DXUTGetGlobalResourceCache().OnResetDevice( pd3dDevice );
    if( FAILED(hr) )
        return DXUT_ERR( L"OnResetDevice", DXUTERR_RESETTINGDEVICEOBJECTS );
...
HRESULT DXUTReset3DEnvironmentWithoutLost( bool bCallLostWhenFailed )
{
	...
	if ( GetDXUTState().GetDeviceObjectsReset() == false )
	{
		hr = DXUTGetGlobalResourceCache().OnResetDevice( pd3dDevice );
		if( FAILED(hr) )
			return DXUT_ERR( L"OnResetDevice", DXUTERR_RESETTINGDEVICEOBJECTS );
			...
			if ( bQueuedMsg == false )
			{
					...
					0 == TranslateAccelerator( hWnd, hAccel, &msg ) )
				{
					...
					if( msg.message == WM_KEYDOWN || msg.message == WM_KEYUP 
						|| msg.message == WM_SYSKEYDOWN || msg.message == WM_SYSKEYUP )
					{
						GetDXUTState().SetVirtualKey( ( msg.wParam == VK_PROCESSKEY ) 
							? ImmGetVirtualKey( hWnd ) : msg.wParam );
					}//if

					GetDXUTState().SetLastPostMessageTime( msg.time );
					//}} robobeg : 2009-03-25
...
#ifdef CLIENT_USE_XTRAP	// XTRAP 클라이언트 - 살아있는지 체크
			CheckAliveXTrap();
#endif CLIENT_USE_XTRAP
		}
	}
        ...
        if( bGotMsg )
        {
                ...
                0 == TranslateAccelerator( hWnd, hAccel, &msg ) )
            {
				...
				else
				{
					...
					{
						//외부에서 들어오는 user message가 성능 측정에 영향을 주지 않도록...
						KOGGamePerformanceCheck::GetInstance()->Pause();
					}
...
//      - Calling Present()
//--------------------------------------------------------------------------------------
#ifdef RENDER_SKIP_TEST
float			g_fConstRenderSkipTime = (1.f/60.f);
float			g_fAddElapsedTimebyOnFrameRender = 0.f;
...
void DXUTRender3DEnvironment()
{
    ...
    {
#ifdef RENDER_SKIP_TEST
		g_fAddElapsedTimebyOnFrameRender += fElapsedTime;
		if( g_fAddElapsedTimebyOnFrameRender < g_fConstRenderSkipTime )
			return;
		else
		{
			g_fAddElapsedTimebyOnFrameRender = 0.f;// g_fConstRenderSkipTime;
		}
#endif RENDER_SKIP_TEST

#ifdef DXUT_KTDXLIB_SAFE_ON_RESET
		DXUTReset3DEnvironmentWithoutLost( false );
#endif
...
#ifdef ACTIVE_KOG_GAME_PERFORMANCE_CHECK
	//시뮬과 렌더링이 모두 끝난뒤에 평균프레임 업데이트한다.
	KOGGamePerformanceCheck::GetInstance()->GetAvgFpsCheckMgr()->UpdateAvgFps();
	KOGGamePerformanceCheck::GetInstance()->GetMemUsageCheckMgr()->UpdateMemUsage();
#endif//ACTIVE_KOG_GAME_PERFORMANCE_CHECK
...
void DXUTUpdateFrameStats()
{
	...
	{
		// Keep track of the frame count
		double fLastTime = GetDXUTState().GetLastStatsUpdateTime2();
		DWORD dwFrames  = GetDXUTState().GetLastStatsUpdateFrames2();
		dwFrames++;
		GetDXUTState().SetLastStatsUpdateFrames2( dwFrames );

		// Update the scene stats once per second
		if( fTime - fLastTime > 1.0f )
		{
			float fFPS = (float) (dwFrames / (fTime - fLastTime));
			GetDXUTState().SetFPS2( fFPS );
			GetDXUTState().SetLastStatsUpdateTime2( fTime );
...
{
#ifdef  CHANGE_KEY_DEVICE
    if ( g_pKTDXApp != NULL && g_pKTDXApp->GetDIManager() != NULL ) 
    {
        g_pKTDXApp->GetDIManager()->MsgProc_KeyMsg( hWnd, uMsg, wParam, lParam );
    }//if
#endif
	...
	if( g_pKTDXApp != NULL && g_pKTDXApp->GetDIManager() != NULL )
	{
		...
		{		
			if( wParam )
			{
				g_pKTDXApp->GetDIManager()->ActivateApp();
			}
			else
			{
				g_pKTDXApp->GetDIManager()->DeactivateApp();
			}
		}
	}	
#endif
    ...
    switch( uMsg )
    {
		...
		case WM_SIZE:
            ...
            {
				//최소화 시에 타이머가 정지해 있으면 다시 최대화가 되지 않는다
...
#ifndef WINDOW_SYSTEM_BUTTON_FUNC
       ...
       case WM_ENTERMENULOOP:
        ...
        case WM_SYSCOMMAND:
            ...
            switch( wParam )
            {
				case SC_PREVWINDOW:		// alt+F6 키 조합을 막자
				case SC_NEXTWINDOW: 
				//case SC_KEYMENU: 
					{
						return 0;		 
				...
				case SC_CLOSE:			// Alt+F4
					{
						//g_pKTDXApp->NoticeQuitType( CKTDXApp::KQT_CLOSE_WINDOW );

						CKTDXStage* pStage = NULL;
						//{{ JHKang / 강정훈 / 2010.09.2 / 윈도우 최소화, 최대화, 종료 버튼 추가 및 메시지 변경 수행
#ifdef WINDOW_SYSTEM_BUTTON_FUNC
						pStage = g_pKTDXApp->GetStage( L"NowState" );
						if ( pStage != NULL )
							pStage->MsgProc( g_pKTDXApp->GetHWND(), CKTDXApp::KM_UI_CONTROL_CUSTOM_EVENT, 3008, NULL );
				...
				case VK_RETURN:
					{
						...
						if( GetDXUTState().GetHandleAltEnter() )
						{
							...
							if( (lParam & dwMask) != 0 ) // Alt is down also
							{
								...
								// note!! X2State에서 이 메세지를 처리하기 때문에 이 부분 코드는 실행되지 않습니다.
								ASSERT( !"NO direct alt+enter process" );
...
void DXUTCleanup3DEnvironment( bool bReleaseSettings )
{
    ...
    if( pd3dDevice != NULL )
    {
        ...
        if( pd3dDevice )
        {
            ...
            {
#ifndef DXUT_KTDXLIB_SAFE_ON_RESET
...
RECT DXUTGetFullscreenClientRectAtModeChange()       { RECT rc = { 0, 0, GetDXUTState().GetFullScreenBackBufferWidthAtModeChange(), GetDXUTState().GetFullScreenBackBufferHeightAtModeChange() }; return rc; }
void DXUTSetWindowClientRectAtModeChange(int width, int height)          { GetDXUTState().SetWindowBackBufferWidthAtModeChange(width); GetDXUTState().SetWindowBackBufferHeightAtModeChange(height); }
void DXUTSetFullscreenClientRectAtModeChange(int width, int height)       { GetDXUTState().SetFullScreenBackBufferWidthAtModeChange(width), GetDXUTState().SetFullScreenBackBufferHeightAtModeChange(height); }
...
#ifdef CHANGE_KEY_DEVICE
//{{ robobeg : 2011-04-08
WORD DXUTGetVirtualKey()
{
	return  GetDXUTState().GetVirtualKey();
}//DXUTGetVirtualKey()


void    DXUTSetVirtualKey( WORD wParam )
{
	GetDXUTState().SetVirtualKey( wParam );
}//

DWORD	DXUTGetLastPostMessageTime()
{
	return	GetDXUTState().GetLastPostMessageTime();
}

void    DXUTSetLastPostMessageTime( DWORD dwTime )
{
	GetDXUTState().SetLastPostMessageTime( dwTime );
}//DXUTSetLastPostMessageTime()
...
//////////////////////////////////////////////////////////////////////////
//## Desc :
HRESULT		DXUTNoCreateWindow(HWND hClienthWnd)
{
	// Not allowed to call this from inside the device callbacks
	if( GetDXUTState().GetInsideDeviceCallback() )
		return DXUT_ERR_MSGBOX( L"DXUTCreateWindow", E_FAIL );
	...
	GetDXUTState().SetHInstance( (HINSTANCE)GetModuleHandle(NULL) );

	GetDXUTState().SetWindowCreatedWithDefaultPositions( true );

	GetDXUTState().SetWindowCreated( true );
	GetDXUTState().SetHWNDFocus( hClienthWnd );
	GetDXUTState().SetHWNDDeviceFullScreen( hClienthWnd );
	GetDXUTState().SetHWNDDeviceWindowed( hClienthWnd );
...
#ifdef VERTICAL_SYNC_OPTION
int DXUTGetVerticalSync()
{
	return GetDXUTState().GetOverrideForceVsync();
}

void DXUTSetVerticalSync( IN int iVal_ )
{
	DXUTDeviceSettings* DeviceSettings = GetDXUTState().GetCurrentDeviceSettings();
...
}//DXUTGetDeviceObjectsReset()
#endif

#ifdef CLIENT_USE_XTRAP
void CheckAliveXTrap()	// XTRAP 클라 - 정해진 시간마다 살아있는지 체크
{
	static DWORD dwExecute = 0;
	if((GetTickCount() - dwExecute) > 2000)
	{
