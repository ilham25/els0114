...
class CKTDGUIIMEEditBox : public CKTDGUIControl
{
	public:
		enum IME_EDITBOX_EVENT_MSG
		{
			IEEM_CHANGE = 0,
			IEEM_ENTER,
		};
	public:
		CKTDGUIIMEEditBox(void);
		virtual	~CKTDGUIIMEEditBox(void);
		...
		virtual	bool	MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
				bool	EBProc( UINT uMsg, WPARAM wParam, LPARAM lParam );
		...
		void	SetCustomMsgChange( int msg ){ m_CustomMsgEditBoxChange = msg; }
		void	SetCustomMsgEnter( int msg ){ m_CustomMsgEditBoxEnter = msg; }
		void	SetCustomMsgFocusOut(int val) { m_CustomMsgEditBoxFocusOut = val; }
		...
		void	RenderCandidateReadingWindow( IDirect3DDevice9* pd3dDevice, float fElapsedTime, bool bReading );
		void	RenderComposition( IDirect3DDevice9* pd3dDevice, float fElapsedTime );

		void	SetText_LUA( const char* pcText );
		...
		void	SetCaretColor( D3DCOLOR Color )			{ m_CaretColor = Color; m_CompCaretColor = Color; }	// Caret color
		void	SetOutlineColor( bool bCheck, D3DCOLOR Color = 0xff000000 )	{ m_bOutlineCheck = bCheck; m_OutlineColor = Color; }
		void	ParseFloatArray( float *pNumbers, int nCount );
		void	SetTextFloatArray( const float *pNumbers, int nCount );

		void	SetTextColor_LUA( float colorR, float colorG, float colorB, float colorA )			{ m_TextColor = D3DCOLOR_COLORVALUE(colorR,colorG,colorB,colorA); }
		void	SetSelectedTextColor_LUA( float colorR, float colorG, float colorB, float colorA )	{ m_SelTextColor = D3DCOLOR_COLORVALUE(colorR,colorG,colorB,colorA); }
		void	SetSelectedBackColor_LUA( float colorR, float colorG, float colorB, float colorA )	{ m_SelBkColor = D3DCOLOR_COLORVALUE(colorR,colorG,colorB,colorA); }
		void	SetCaretColor_LUA( float colorR, float colorG, float colorB, float colorA )		{ m_CaretColor = D3DCOLOR_COLORVALUE(colorR,colorG,colorB,colorA); m_CompCaretColor = m_CaretColor; }
		void	SetOutlineColor_LUA( float colorR, float colorG, float colorB, float colorA )		{ m_OutlineColor = D3DCOLOR_COLORVALUE(colorR,colorG,colorB,colorA); m_bOutlineCheck = true; }
		void	SetLengthLimit_LUA( int iLengthLimit ) { m_Buffer.SetLengthLimit( iLengthLimit ); }
		void	SetOneCharMode_LUA( bool val ) { m_Buffer.SetOneCharMode( val ); }
		void	SetOnlyNumMode_LUA( bool val ) { m_Buffer.SetOnlyNumMode( val ); }
		void	SetOnlyNumAndEnglishMode_LUA( bool val, bool bCheckBigAndSmall, bool bBigEnglish ) { m_Buffer.SetOnlyNumAndEnglishMode( val, bCheckBigAndSmall, bBigEnglish ); }
		void	SetAutoChangeBigEnglish( bool val ) { m_Buffer.SetAutoChangeBigEnglish( val ); }
		void	SetDefTextAlign_LUA( DWORD val ) { m_DefTextAlign = val; }
        void	SetWidthLimit_LUA( int _width ) { m_Buffer.SetWidthLimit( _width ); }
        void	SetLineLimit_LUA( int _Height ) { m_Buffer.SetLineLimit( _Height ); }
		void	SetByteLimit_LUA( int val ) { m_Buffer.SetByteLimit( val ); }

		void	SetMultiLineOption_LUA( int groupID, int groupSequence );
		int		GetGroupID() { return m_GroupID; }
		int		GetGroupSequence() { return m_GroupSequence; }
...
protected:
	...
	protected:
		...
		static	void	GetPrivateReadingString();
				...
				HRESULT CalcTextRect( LPCWSTR strText, RECT* prcDest, int nCount = -1, DWORD format = (DT_VCENTER | DT_CALCRECT) );
				void UpdateTextAlignOffset();

				bool CheckTextOversize();
				HRESULT	DrawRect( RECT* prcDest, D3DCOLOR color );

				bool	ContainPTForIME(POINT pt);

			CKTDGUIIMEEditBox* GetNextIMEEditBox();
			CKTDGUIIMEEditBox* GetPrevIMEEditBox();
	...
	protected:
		...
		struct CCandList
		{
			...
			CKTDGUIUniBuffer	HoriCand;	// Candidate list string (for horizontal candidate window)
		...
		static	CKTDGUIUniBuffer	s_CompString;			// Buffer to hold the composition string (we fix its length)
		...
		static	bool		s_bCreateDeviceCheck;
		static	CGrowableArray< CInputLocale > s_Locale; // Array of loaded keyboard layout on system
				...
				DWORD		m_DefTextAlign;			// 텍스트 정렬
				POINT		m_ptTextAlignOffset;	// 텍스트 정렬 오프셋
